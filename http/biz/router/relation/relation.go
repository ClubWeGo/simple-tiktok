	...
	root := r.Group("/", rootMw()...)
	{
		_douyin := root.Group("/douyin", _douyinMw()...)
		{
			_message := _douyin.Group("/message", _messageMw()...)
			{
				...
				_action.POST("/", append(_message_ctionmethodMw(), relation.MessageActionMethod)...)
			}
			{
				_chat := _message.Group("/chat", _chatMw()...)
				_chat.POST("/", append(_messagechatmethodMw(), relation.MessageChatMethod)...)
			}
		}
		{
			_relation := _douyin.Group("/relation", _relationMw()...)
			{
				...
				_action0.POST("/", append(_relationmethodMw(), relation.RelationMethod)...)
			}
			{
				_follow := _relation.Group("/follow", _followMw()...)
				{
					_list := _follow.Group("/list", _listMw()...)
					_list.GET("/", append(_followlistmethodMw(), relation.FollowListMethod)...)
				}
			}
			{
				_follower := _relation.Group("/follower", _followerMw()...)
				{
					...
					_list0.GET("/", append(_followerlistmethodMw(), relation.FollowerListMethod)...)
				}
			}
			{
				_friend := _relation.Group("/friend", _friendMw()...)
				{
					...
					_list1.GET("/", append(_friendlistmethodMw(), relation.FriendListMethod)...)
				}
			}
		}
	}
